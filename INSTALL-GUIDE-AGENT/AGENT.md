...
| First turn works, then a later turn (tool-loop step or follow-up message) fails with `{"detail":"Unsupported parameter: previous_response_id"}` | Codex (>= ~0.142) chains that turn with `previous_response_id`, assuming a persistent upstream session; every round this proxy opens is a fresh, unrelated HTTP request, so upstream can't resolve it | Update CodexCont — the proxy resolves `previous_response_id` from a local per-process cache (splices the cached history, always drops the field before forwarding) instead of passing it upstream. If it recurs on a current checkout, it's almost always a cache miss (proxy restarted mid-session, or the id fell out of the TTL/size-bounded cache) — restarting the agent's session works around it. |
